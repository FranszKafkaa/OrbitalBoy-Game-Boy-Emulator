...
void PPU::tick(u32 cycles, std::array<u8, 0x2000>& vram, std::array<u8, 0xA0>& oam) {
    if ((lcdc_ & 0x80) == 0) {
        ly_ = 0;
        mode_ = 0;
        modeClock_ = 0;
...
void PPU::write(u16 address, u8 value) {
    ...
    case 0xFF41: stat_ = static_cast<u8>((stat_ & 0x07) | (value & 0x78)); break;
    case 0xFF42: scy_ = value; break;
    case 0xFF43: scx_ = value; break;
    case 0xFF44: ly_ = 0; break;
...
void PPU::renderScanline(const std::array<u8, 0x2000>& vram, const std::array<u8, 0xA0>& oam) {
    if (ly_ >= ScreenHeight) {
        return;
    }
    ...
    if (bgEnabled) {
        ...
        for (int x = 0; x < ScreenWidth; ++x) {
            ...
            const u8 tileNum = vram[tileIdxAddress & 0x1FFF];
            ...
            const u8 line = static_cast<u8>((y % 8) * 2);
            const u8 lo = vram[(tileAddr + line) & 0x1FFF];
            const u8 hi = vram[(tileAddr + line + 1) & 0x1FFF];

            const u8 bit = static_cast<u8>(7 - (px % 8));
    ...
    if (windowEnabled && ly_ >= wy_) {
        ...
        if (windowXStart < ScreenWidth) {
            ...
            for (int x = 0; x < ScreenWidth; ++x) {
                ...
                const u8 line = static_cast<u8>((windowY % 8) * 2);
                const u8 lo = vram[(tileAddr + line) & 0x1FFF];
                const u8 hi = vram[(tileAddr + line + 1) & 0x1FFF];
                const u8 bit = static_cast<u8>(7 - (wxPixel % 8));
                const u8 colorId = static_cast<u8>(((hi >> bit) & 0x01) << 1 | ((lo >> bit) & 0x01));
                bgColorId[x] = colorId;
                framebuffer_[ly_ * ScreenWidth + x] = paletteMap(bgp_, colorId);
            }
    ...
    for (int i = 0; i < 40 && drawn < 10; ++i) {
        ...
        const u8 lo = vram[tileAddr & 0x1FFF];
        const u8 hi = vram[(tileAddr + 1) & 0x1FFF];
