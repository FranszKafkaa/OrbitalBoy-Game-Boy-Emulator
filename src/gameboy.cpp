...
bool writeBusState(std::ostream& os, const Bus::State& s) {
    return writeArray(os, s.vram) && writeArray(os, s.wram) && writeArray(os, s.oam)
        && writeArray(os, s.hram) && writeTimerState(os, s.timer)
        && writeJoypadState(os, s.joypad) && writePpuState(os, s.ppu)
        && writeApuState(os, s.apu) && writePod(os, s.ie) && writePod(os, s.iflag);
}

bool readBusState(std::istream& is, Bus::State& s) {
    return readArray(is, s.vram) && readArray(is, s.wram) && readArray(is, s.oam)
        && readArray(is, s.hram) && readTimerState(is, s.timer)
        && readJoypadState(is, s.joypad) && readPpuState(is, s.ppu)
        && readApuState(is, s.apu) && readPod(is, s.ie) && readPod(is, s.iflag);
}
...
bool GameBoy::loadRom(const std::string& path) {
    return cartridge_.loadFromFile(path);
}
